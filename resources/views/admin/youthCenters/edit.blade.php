...
@section('content')
    <div class="card">
        ...
        <form action="{{ route('admin.youthCenters.update', ['youthCenter' => $youthCenter->id]) }}"
                    ...
                    <div class="col-md-1">
                        <select name="status" class="form-control  service status">
                            @foreach (Constants::getAllStatus() as $status)
                                <option {{ $youthCenter->status == $status['value'] ? 'selected' : '' }}
                    ...
                    <div class="col-md-12">
                        <div class="col-md-4">
                            <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                                ...
                                <input id="name" type="text" class="form-control" value="{{ $youthCenter->name }}"
                                    name="name">
                                @if ($errors->has('name'))
                        ...
                        <div class="col-md-4">
                            <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                                ...
                                <input id="adress" type="text" class="form-control"
                                    value="{{ $youthCenter->address }}" name="address">
                            ...
                            <div class="form-group {{ $errors->has('city_id') ? 'has-error' : '' }}">
                                ...
                                <select name="city_id" id="city_id" class="form-control select2" required>
                                    @forelse ($cities as $city)
                                        <option {{ $city->id == $youthCenter->city_id ? 'selected' : '' }}
                ...
                <div class="row" style="max-height: 700px; overflow-y: auto;">
                    <table class="table youth_center_services_table">
                        <thead>
                            ...
                            <td>{{ trans('cruds.youth_centers.fields.services.duration') }} (minutes)</td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="display_{{ $service->id }}">
                                            <input type="number" class="form-control"
                                                value="{{ $is_pivot ? $service->pivot->duration : $service->duration }}"
                                    ...
                                    <td>
                                        <span class="display_{{ $service->id }}">
                                            ...
                                            <input type="number" class="form-control"
                                                value="{{ $is_pivot ? $service->pivot->max_places : $service->max_places }}"
